...
function hideBasedOnSelected(dropdownList){
    ...
    const valueParent = dropdownList.parentElement;
    for(const card of valueParent.querySelectorAll('.card')) {
        card.classList.add('hide');
    }
    if(value == 'egyeb' && !valueParent.querySelector('#egyeb')) {
        createOtherDivForSelect(valueParent);
    }
    makeVisibleBasedOnId(valueParent, value);
}
